// Daily generation-cap helpers. Pure and server-free so they can be unit-tested
// without booting the Express app or connecting to Supabase.

const DEFAULT_LIMIT = 10;
const DEFAULT_TZ = "Australia/Sydney";

export function dailyLimit(): number {
  const n = Number(process.env.DAILY_GENERATION_LIMIT);
...
/** UTC Date of the most recent local midnight in `tz` relative to `now`.
 *  DST-safe: uses a two-step refinement so the two DST-transition days per year
 *  are handled correctly (ported faithfully from legacy/usage.js). */
