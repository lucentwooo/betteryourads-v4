...
-- profiles: one row per auth user. `approved` gates app access; it now defaults
-- to true so testers can self-serve immediately (set false to ban an account).
-- The per-account lifetime free cap (LIFETIME_GENERATION_LIMIT) limits spend;
-- flip `is_admin` to true to grant a tester unlimited generations.
