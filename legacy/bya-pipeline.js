 ...
 *
 * Wraps the same server endpoints the original app (now admin.html) uses, so the
 * customer UI never re-implements the wiring. Depends on:
...
(function () {
  ...
  async function generateImage(args, onStatus) {
    ...
    return { ok: false, error: "Timed out after 2 minutes. The render may still finish — check My ads shortly." };
  }
  ...
  async function loadBrands() {
    ...
    const { data, error } = await c.from("brands").select("id,name,website_url,analysis,updated_at").order("updated_at", { ascending: false });
    if (error) throw new Error(error.message);
    return data || [];
  }
  async function saveBrand(websiteUrl, analysisObj, userId) {
    const c = authed().client; if (!c || !userId) return null;
    let host = websiteUrl; try { host = new URL(websiteUrl).hostname; } catch (e) {}
    const { data, error } = await c.from("brands").upsert(
      { user_id: userId, name: host, website_url: websiteUrl, analysis: analysisObj, updated_at: new Date().toISOString() },
  ...
  window.BYA = {
    ...
    generateAngles: generateAngles, generateImage: generateImage, saveAd: saveAd,
    // data
