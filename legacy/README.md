...
> **Important:** open that exact URL — do NOT double-click the `.html` files (a `file://` page
...
The app ships with a `Dockerfile` (built on the official Playwright image, so Chromium and
all its system libraries are baked in) and a `render.yaml` Blueprint.

1. Push this repo to GitHub.
2. In Render: **New → Blueprint**, select the repo. Render reads `render.yaml` and creates a
   Docker web service.
3. When prompted, fill in the secret env vars (left blank in the Blueprint):
   `OPENROUTER_API_KEY`, `KIE_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`,
   `SUPABASE_SERVICE_ROLE_KEY`. The non-secret model settings are pre-filled and editable in
   the dashboard.
4. Deploy. Render injects `PORT` automatically; `server.js` already binds to it.

> **Plan note:** the Blueprint uses the **Starter** plan because headless Chromium needs more
> than the free plan's 512 MB and will OOM on `/extract` otherwise.

> **Supabase redirect:** once you know your Render URL (e.g. `https://betteryourads.onrender.com`),
> go to Supabase **Authentication → URL Configuration** and set **Site URL** + add a
> **Redirect URL** for that domain, or magic-link sign-in will fail in production.
